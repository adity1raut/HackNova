...
router.get("/api/getCurrentLecture/:department/:email", async (req, res) => {
    try {
        ...
        console.log(`Today is: ${currentDay}, Current Time: ${allTimeTables}`);

        for (const timeTable of (Array.isArray(allTimeTables) ? allTimeTables : [allTimeTables])) {
            ...
            if (timeTable[currentDay]) {
                for (const slot of timeTable[currentDay]) {
                    ...
                    if (
                        ...
                        let yearModified;
                        if(timeTable.year === "first") {
                            yearModified = "FE";
                        }else if(timeTable.year === "second") {
                            yearModified = "SE";
                        }else if(timeTable.year === "third") {
                            yearModified = "TE";
                        }else{
                            yearModified = "BE";
                        }
                        const allStudents = await User.find({ branch: department, year: yearModified });
